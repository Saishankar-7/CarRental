...
const Navbar = ({}) => {
  ...
  return (
    ...
    xl:px-32 py-4 text-gray-600 border-borderColor relative transition-all
    ${location.pathname === '/' && 'bg-light'}`}>
     ...
     max-sm:border-t border-borderColor right-0 flex flex-col sm:flex sm:flex-row
     ...
     duration-300 z-50 ${location.pathname === '/' ? 'bg-light' : 'bg-white'} ${open ? 'max-sm:translate-x-0' : 'max-sm:translate-x-full'}`}>
        ...
        <div className='hidden lg:flex items-center text-sm gap-2 border
         border-boderColor px-3 rounded-full max-w-56'>
            <input type="text" className='py-1.5 w-full bg-transperent outline-none
        ...
        <div className='flex max-sm:flex-col items-start sm:items-center gap-6'>
            ...
            onClick={()=>{user ? logout():setShowLogin(true)}}className='cursor-pointer px-8 py-2 bg-primary
            hover:bg-primary-dull transition-all text-white rounded-lg'>{user ?'LogOut':'Login'}</motion.button>
        </div>
     </div>
